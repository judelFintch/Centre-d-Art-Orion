...
@section('meta_description', $activeCategory ? __('pages.blog.meta_description_category', ['category' => $activeCategory['name']]) : __('pages.blog.meta_description_default'))
...
<section style="background:#0a0a0a;position:relative;overflow:hidden;padding:120px 0 72px;border-bottom:1px solid #1a1a1a;">
    ...
    <div style="max-width:1280px;margin:0 auto;padding:0 24px;position:relative;z-index:1;">
        <div style="display:grid;grid-template-columns:minmax(0,0.9fr) minmax(420px,1.1fr);gap:48px;align-items:end;">
            <div>
                <div class="tag tag-gold" style="margin-bottom:18px;">{{ $activeCategory ? __('pages.blog.tag_category') : __('pages.blog.tag_default') }}</div>
                <h1 style="font-family:'Playfair Display',Georgia,serif;font-size:clamp(2.8rem,7vw,6.2rem);font-weight:900;color:#f5f5f0;line-height:0.95;margin:0 0 24px;">
                    {{ $activeCategory ? $activeCategory['name'] : __('pages.blog.title_default') }}<br>
                    <span style="background:linear-gradient(135deg,#d4a030,#4caf7d);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;">{{ $activeCategory ? __('pages.blog.title_gradient_category') : __('pages.blog.title_gradient_default') }}</span>
                </h1>
                <p style="color:#8f877d;font-size:1.05rem;line-height:1.9;max-width:620px;margin:0;">
                    {{ $activeCategory ? __('pages.blog.desc_category', ['count' => $activeCategory['count']]) : __('pages.blog.desc_default') }}
                </p>
            </div>
            ...
            <a href="{{ route('blog.show', $featured['slug']) }}"
               style="min-height:420px;background:#111;border:1px solid #1f1a12;border-radius:8px;overflow:hidden;display:flex;flex-direction:column;justify-content:flex-end;position:relative;text-decoration:none;">
                ...
                <div style="position:relative;padding:34px;">
                    <span class="tag tag-gold">{{ __('pages.blog.featured_badge') }}</span>
                    ...
                    <span style="color:#d4a030;font-family:'Space Grotesk',sans-serif;font-size:0.82rem;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;">{{ __('pages.blog.read_article') }}</span>
                </div>
            </a>
            @endif
        </div>
...
<section style="padding:72px 0 100px;background:#0d0d0d;">
    <div style="max-width:1280px;margin:0 auto;padding:0 24px;">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:20px;margin-bottom:32px;flex-wrap:wrap;">
	            <h2 style="font-family:'Space Grotesk',sans-serif;font-size:1rem;font-weight:700;color:#f5f5f0;letter-spacing:0.1em;text-transform:uppercase;margin:0;">{{ $activeCategory ? __('pages.blog.filtered_articles') : __('pages.blog.all_articles') }}</h2>
            ...
            <a href="{{ route('blog.show', $article['slug']) }}"
               ...
               style="background:#111;border:1px solid #1a1a1a;border-radius:8px;overflow:hidden;text-decoration:none;display:flex;flex-direction:column;transition:all 0.3s;">
                ...
                <div style="padding:24px;display:flex;flex-direction:column;flex:1;">
                    ...
                    <span style="color:#4caf7d;font-family:'Space Grotesk',sans-serif;font-size:0.78rem;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;">{{ __('pages.blog.open_article') }}</span>
                </div>
            </a>
            @endforeach
        </div>
...
<section style="padding:72px 24px;background:#0d1a12;border-top:1px solid #1a2e20;border-bottom:1px solid #1a2e20;">
    <div style="max-width:640px;margin:0 auto;text-align:center;">
        <p style="font-family:'Space Grotesk',sans-serif;font-size:0.72rem;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;color:#4caf7d;margin:0 0 12px;">{{ __('pages.blog.newsletter_tag') }}</p>
        <h2 style="font-family:'Playfair Display',Georgia,serif;font-size:2rem;font-weight:700;color:#f5f5f0;margin:0 0 14px;line-height:1.2;">{{ __('pages.blog.newsletter_title') }}</h2>
        <p style="color:#666;font-size:0.9rem;line-height:1.75;margin:0 0 32px;">{{ __('pages.blog.newsletter_desc') }}</p>
        ...
        <p style="color:#444;font-size:0.78rem;margin:18px 0 0;">{{ __('pages.blog.newsletter_note') }}</p>
    </div>
</section>
