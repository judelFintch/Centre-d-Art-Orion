...
@section('title', 'Modifier un élément galerie')
...
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;gap:12px;flex-wrap:wrap;">
    ...
    <a href="{{ route('galerie.index') }}" target="_blank" style="color:#d4a030;font-family:'Space Grotesk',sans-serif;font-size:0.82rem;font-weight:600;text-decoration:none;">Voir sur le site →</a>
</div>
...
<form method="POST" action="{{ route('admin.galerie.update', $galerie) }}" enctype="multipart/form-data">
    ...
    <div style="display:flex;justify-content:space-between;gap:12px;margin-top:28px;padding-top:24px;border-top:1px solid #1a1a1a;flex-wrap:wrap;">
        <button type="submit" form="delete-galerie-item" style="padding:10px 18px;background:rgba(224,112,48,0.08);border:1px solid rgba(224,112,48,0.25);color:#e07030;font-family:'Space Grotesk',sans-serif;font-size:0.82rem;font-weight:700;border-radius:6px;cursor:pointer;">Supprimer</button>
...
<form id="delete-galerie-item" method="POST" action="{{ route('admin.galerie.destroy', $galerie) }}" onsubmit="return confirm('Supprimer cet élément ?')" style="display:none;">
    @csrf
    @method('DELETE')
</form>
