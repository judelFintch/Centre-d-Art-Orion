...
@section('title', __('pages.ticketing.title'))
@section('meta_description', __('pages.ticketing.meta_description'))
...
<section style="background:linear-gradient(135deg,#0a0a0a 0%,#111 100%);padding:120px 0 72px;text-align:center;border-bottom:1px solid #1a1a1a;">
    <div style="max-width:800px;margin:0 auto;padding:0 24px;">
        <p style="font-family:'Space Grotesk',sans-serif;font-size:0.72rem;font-weight:700;letter-spacing:0.18em;text-transform:uppercase;color:#4caf7d;margin:0 0 16px;">{{ __('pages.ticketing.hero_kicker') }}</p>
        <h1 style="font-family:'Playfair Display',serif;font-size:clamp(2rem,5vw,3.5rem);font-weight:900;color:#f5f5f0;margin:0 0 20px;line-height:1.15;">{{ __('pages.ticketing.hero_title') }}</h1>
        <p style="font-size:1.05rem;color:#888;line-height:1.7;max-width:560px;margin:0 auto;">{{ __('pages.ticketing.hero_desc') }}</p>
    </div>
</section>
...
<section style="padding:80px 0;background:#faf8f4;">
    <div style="max-width:1200px;margin:0 auto;padding:0 24px;">
        ...
        <div style="text-align:center;padding:80px 24px;">
            ...
            <h2 style="font-family:'Playfair Display',serif;font-size:1.6rem;color:#1c1510;margin:0 0 12px;">{{ __('pages.ticketing.empty_title') }}</h2>
            <p style="color:#888;font-size:0.95rem;">{{ __('pages.ticketing.empty_desc') }}</p>
            <a href="{{ route('evenements.index') }}" style="display:inline-block;margin-top:24px;padding:10px 24px;background:linear-gradient(135deg,#4caf7d,#2d7a52);color:#fff;font-family:'Space Grotesk',sans-serif;font-size:0.82rem;font-weight:600;letter-spacing:0.06em;text-transform:uppercase;border-radius:4px;text-decoration:none;">{{ __('pages.ticketing.empty_cta') }}</a>
        </div>
        @else
            ...
            <article style="background:#fff;border:1px solid #ede8e0;border-radius:12px;overflow:hidden;display:flex;flex-direction:column;transition:box-shadow 0.3s,transform 0.3s;"
                ...
                <div style="padding:24px;display:flex;flex-direction:column;flex:1;">
                    ...
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;flex-wrap:wrap;">
                        ...
                        @endif
                        @if($evt->gratuit)
                        <span style="padding:3px 10px;background:rgba(76,175,125,0.15);color:#4caf7d;font-family:'Space Grotesk',sans-serif;font-size:0.68rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;border-radius:20px;">{{ __('pages.ticketing.free') }}</span>
                        @else
                    ...
                    <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px;">
                        ...
                        <span style="font-family:'Space Grotesk',sans-serif;font-size:0.8rem;color:#666;">{{ $evt->date_debut->isoFormat('dddd D MMMM YYYY ['.__('pages.ticketing.at_time').'] HH:mm') }}</span>
                    </div>
...
<section style="background:#0a0a0a;padding:72px 24px;text-align:center;">
    <p style="font-family:'Space Grotesk',sans-serif;font-size:0.72rem;font-weight:700;letter-spacing:0.18em;text-transform:uppercase;color:#4caf7d;margin:0 0 12px;">{{ __('pages.ticketing.help_kicker') }}</p>
    <h2 style="font-family:'Playfair Display',serif;font-size:1.8rem;color:#f5f5f0;margin:0 0 16px;">{{ __('pages.ticketing.help_title') }}</h2>
    <p style="color:#666;font-size:0.92rem;margin:0 0 28px;max-width:500px;margin-left:auto;margin-right:auto;">{{ __('pages.ticketing.help_desc') }}</p>
